			...
			if (choice == 1) {
				...
				if (choice == 1) {
					...
					while (true) {
						...
						if (choice == 1) {
								//범인과의 대화 - 기본정보
							System.out.println(dto.nickname()+"상민 학생에게 질문을 한다.");
							System.out.println(dto.nickname()+" : "+"당신이 PBK 사체 최초발견자 입니까?");
							System.out.println("상민 :네... 10시 40분 넘어서 옥상에 갔더니 PBK선생님을...봤습니다....");
//							 범인과의 대화
//							 boolean true false 반환
//							 true(증거가 있을 때)
							// 추가정보
							System.out.println(dto.nickname()+" : "+"옥상에는 왜 올라가셨나요?");
							System.out.println("상민 : PBK선생님 발표 자료를 준비하다가 바람좀 쐴겸 올라갔는데...");
							System.out.println(dto.nickname()+" : "+"평소 피해자와의 관계가 어떻습니까?");
							System.out.println("상민 : 네? 지금 저를 의심하시나요? 저는 선생님을 도와 많은 프로젝트 진행을 했습니다..");
							System.out.println("저에게 좋은 일자리를 주시기로 약속해서 제가 무급으로 일하고 있는데 선생님을 죽이면");
							...
							System.out.println(dto.nickname()+" : "+"9시 30분부터 10시까지 어디에서 무엇을 했습니까?");
							System.out.println("상민 : 8시 40분 부터 시체 발견 전까지 저는 여기서 혼자 작업중이었습니다.. ");
							System.out.println(dto.nickname()+" : "+"(혼자서 작업했다... 증인은 따로 없군.)");
							System.out.println("상민 : 평소에 박나영원장님이랑 PBK선생님이 논쟁하는 것을 봤습니다... 한번 사람들에게 물어보세요 형사님.. ");
							System.out.println(dto.nickname()+" : "+"(박나영 원장과도 이야기를 해야겠군..)");
					...
					if (choice == 1) {
						// 범인과의 대화
						//범인과의 대화 - 기본정보
					System.out.println(dto.nickname()+"박나영 원장에게 질문을 한다.");
					System.out.println(dto.nickname()+" : "+"원장님, 질문 몇개 해도 되겠습니까?(오... 온갖 명품에.. 매우 화려한 스타일이군..");
					System.out.println("나영 : (앙칼지게) 지금 저를 의심하시는건가요? 좋아요. 뭐든지 물어보세요.");
//					 범인과의 대화
//					 boolean true false 반환
//					 true(증거가 있을 때)
					// 추가정보
					System.out.println(dto.nickname()+" : "+"피해자는 어떤 사람입니까?");
					System.out.println("나영 : 좀 건방지긴 하지만 일은 잘했어요. 그래서 승진도 빨랐구요.");
					System.out.println(dto.nickname()+" : "+"평소 피해자와의 관계가 어떻습니까?");
					System.out.println("나영 : 솔직하게 말하면 좀 재수가 없었죠. 제 결정에 간섭을 많이 했거든요.");
					System.out.println("하지만 그런일로 사람을 죽이진 않아요. 저는 가진게 많은 사람이라 굳이 그럴필요도 없고요.");
					System.out.println(dto.nickname()+" : "+"9시 30분부터 10시까지 어디에서 무엇을 했습니까?");
					System.out.println("나영 : 저는 계속 원장실에 있었어요. 중요한 프로젝트가 곧 있어서 시청공무원과 원장실에서 회의중이었어요.");
					System.out.println(dto.nickname()+" : "+"(증인이 있군...하지만 다른 트릭이 있을수도 있어!)");
					System.out.println("나영 : 저는 그 경비원이 의심가요. 이름은 모르겠는데, 그 사람 수상해.");
					System.out.println(dto.nickname()+" : "+"경비원? 이 건물의 경비원이요?");
				...
				if (choice == 1) {
					// 범인과의 대화
					//범인과의 대화 - 기본정보
				System.out.println(dto.nickname()+"장현도 경비에게 질문을 한다.");
				System.out.println(dto.nickname()+" : "+"장현도씨, 질문 몇개 해도 되겠습니까?(덩치가 크고... 생각보다 젊고 깔끔하군.)");
				System.out.println("현도 : (피곤한 표정) 무슨일이십니까..? 저도 힘들어요.. 제친구가 죽었다고요.");
//				 범인과의 대화
//				 boolean true false 반환
//				 true(증거가 있을 때)
				// 추가정보
				System.out.println(dto.nickname()+" : "+"피해자와의 관계는 어떻습니까?");
				System.out.println("현도 : 저는 PBK와 초등학교떄 부터 친구입니다. 저는 절대 범인이 아니에요.");
				System.out.println(dto.nickname()+" : "+"그렇군요, 9시 반부터 10시까지 무엇을 하셨죠?");
				...
				System.out.println(dto.nickname()+" : "+"서하라 선생님과 PBK 사이가 좋지 않나요?");
				System.out.println("현도 : 네, 최근에도 둘 다 승진하려고 애썼는데, PBK만 된걸로 알고 있습니다. ");
				System.out.println(dto.nickname()+" : "+"(일단 차분해 보이는 군... 하지만 다른 무언가는 없을까?)");
				System.out.println("현도 : 저는 PBK의 도움을 많이 받았습니다. 절대 범인이 아닙니다.");
				System.out.println(dto.nickname()+" : "+"혹시 옥상 문은 항상 잠겨있습니까? 열쇠 관리도 하시나요?");
				...
				if (choice == 1) {
					// 범인과의 대화
					System.out.println(dto.nickname()+"서하라 선생님에게 질문을 한다.");
					System.out.println(dto.nickname()+" : "+"서하라씨, 질문 몇개 해도 되겠습니까?(불만이 많아보이는 군)");
...
//					 true(증거가 있을 때)
				...
				if (choice == 1) {
					// 범인과의 대화
					System.out.println(dto.nickname()+"김지은 선생님에게 질문을 한다.");
					System.out.println(dto.nickname()+" : "+"김지은씨, 질문 몇개 해도 되겠습니까?(화장이 진하네..)");
...
//					 true(증거가 있을 때)
					...
					System.out.println(dto.nickname()+" : "+"9시 반부터 10시까지 무엇을 하셨죠?");
					System.out.println("지은 : 저 사실... 9시 20분에 PBK 선생님을 만나서... 선물을 줬어요! 그떄까지는 살아계셨다구요!");
					System.out.println("저는... 상민학생이 의심가요.. 사실 PBK떄문에 너무 고생을 많이 했거든요..");
					System.out.println(dto.nickname()+" : "+"흠.. 그렇다면 피해자의 살아있는 마지막 순간의 목격자시군요.");
					System.out.println("지은 : 그렇게 말씀하시니 좀 위로가 되네요.. 제가 선생님의 마지막 순간을 보다니.. 조금 기뻐요..");
					System.out.println(dto.nickname()+" : "+"(뭐지..싸이코인가?)... 흠 그리고 어딜 가셨죠? 그 이후에");
					System.out.println("지은 : 저 바로 교무실에 돌아와 계속 작업했어요. 제 컴퓨터 내역을 보셔도 좋아요!");
					System.out.println(dto.nickname()+" : "+"(이 사람도.. 조금 의심이 되는군) 현도씨와 만나셨나요?");
